...
These files are proposed for later batches and are not created in this batch.

| File | Purpose | Depends On | Should Modify Existing Flow? |
| --- | --- | --- | --- |
| `agent/model_providers/openai_compatible.py` | Implement an OpenAI-compatible provider adapter that conforms to `BaseModelProvider`. | `agent/model_providers/base.py`, existing `llm.openai_compatible` client patterns | No. It should be opt-in through config/tests. |
| `config/maas_providers.yaml` | Define MaaS provider candidates, base URL, API key env var, model examples, enabled flag, and verification metadata. | Existing config loading patterns | No. It should not replace `config/model_providers.yaml` initially. |
| `scripts/run_maas_provider_smoke_test.py` | Run provider availability/smoke checks with skipped status when API keys are missing. | Provider registry, OpenAI-compatible adapter, recovery decisions | No. It should be a standalone script. |
...
| `docs/MAAS_PROVIDER_ONBOARDING.md` | Document how to add a provider safely, including env vars and verification boundaries. | This plan, provider config | No. Documentation only. |
...
| `tests/test_openai_compatible_maas_provider.py` | Offline tests for adapter config, skipped missing key behavior, parsing, usage, errors, and secret redaction. | Adapter and mock client factory | No. Tests only. |
