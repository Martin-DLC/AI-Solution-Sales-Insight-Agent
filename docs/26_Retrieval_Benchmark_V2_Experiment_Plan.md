...
一旦 7 个正式结果已经发布，`--formal-readiness` 必须返回：

- `ready_for_single_formal_run = false`
- `readiness_reasons` 包含 `formal_results_already_exist`

它不能复用“首次正式运行前必须不存在结果文件”的异常路径直接中止。
...
`--validate`、`--formal-readiness` 与 `--check` 必须分离三层生命周期语义：

1. Core Benchmark Validation：
   只校验数据、配置、数量、Feasibility 和方法合同，不检查正式结果是否存在。
2. Pre-run Readiness：
   在 Core Validation 基础上额外要求 7 个正式结果文件全部不存在，且离线模型与目录状态满足首次正式发布条件。
3. Post-publish Formal Check：
   当 7 个正式结果全部存在时执行严格重算与逐字段比较；当全部不存在时返回 `formal_results_not_generated`；当只存在部分文件时返回 `partial_formal_results_detected`。
