...
def test_full_path_node_order_has_eleven_nodes() -> None:
    snapshot = run_architecture_c_skeleton(
        dev_01_case(),
        WorkflowServices(llm=FakeWorkflowLLMClient.with_default_batch2b_responses()),
    )
...
def test_full_path_fake_llm_total_calls_is_seven() -> None:
    client = FakeWorkflowLLMClient.with_default_batch2b_responses()
...
def test_seven_llm_nodes_called_once() -> None:
    client = FakeWorkflowLLMClient.with_default_batch2b_responses()
...
def test_does_not_generate_ai_opportunities() -> None:
    snapshot = run_architecture_c_skeleton(
        dev_01_case(),
        WorkflowServices(llm=FakeWorkflowLLMClient.with_default_batch2b_responses()),
    )

    assert not hasattr(snapshot, "ai_opportunities")
