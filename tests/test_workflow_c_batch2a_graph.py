...
def test_success_node_order_has_ten_nodes() -> None:
    snapshot = run_architecture_c_skeleton(
        dev_01_case(),
        WorkflowServices(llm=FakeWorkflowLLMClient.with_default_batch2a_responses()),
    )
...
def test_fake_llm_total_calls_is_six() -> None:
...
def test_does_not_generate_information_gaps() -> None:
    snapshot = run_architecture_c_skeleton(
        dev_01_case(),
        WorkflowServices(llm=FakeWorkflowLLMClient.with_default_batch2a_responses()),
    )

    assert not hasattr(snapshot, "information_gaps")
