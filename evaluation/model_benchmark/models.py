...
class NodeBenchmarkRunResult(StrictBaseModel):
    ...
    @classmethod
    def estimated_cost_must_be_non_negative(cls, value: float | None) -> float | None:
        if value is not None and value < 0:
            raise ValueError("Estimated benchmark cost must be zero or greater.")
        return value
...
class NodeModelBenchmarkSummary(StrictBaseModel):
    ...
    @classmethod
    def estimated_total_cost_must_be_non_negative(cls, value: float | None) -> float | None:
        if value is not None and value < 0:
            raise ValueError("Estimated total cost must be zero or greater.")
        return value
...
class NodeExecutionObservation(StrictBaseModel):
    ...
    def estimated_cost_must_be_non_negative(cls, value: float | None) -> float | None:
        if value is not None and value < 0:
            raise ValueError("Observation estimated cost must be zero or greater.")
        return value
